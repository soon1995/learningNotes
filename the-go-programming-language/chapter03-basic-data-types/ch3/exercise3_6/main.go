...
func main() {
	mandelbrotpng, _ := os.OpenFile("mandelbrot.png", os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0666)
	Mandelbrot(mandelbrotpng, mandelbrot)
	mandelbrotpng.Close()
	acospng, _ := os.OpenFile("acos.png", os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0666)
	Mandelbrot(acospng, acos)
	acospng.Close()
	sqrtpng, _ := os.OpenFile("sqrt.png", os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0666)
	Mandelbrot(sqrtpng, sqrt)
	sqrtpng.Close()
	newtonpng, _ := os.OpenFile("newton.png", os.O_WRONLY|os.O_TRUNC|os.O_CREATE, 0666)
	Mandelbrot(newtonpng, newton)
	newtonpng.Close()
}

func Mandelbrot(writer io.Writer, f func(complex128) color.Color) {
	const (
		...
		for px := 0; px < width; px++ {
			...
			for i := 0; i < 2; i++ {
				for j := 0; j < 2; j++ {
					...
					subPixels = append(subPixels, f(z))
				}
			}
