...
func indexComics(comics chan Comic) (WordIndex, NumIndex) {
	...
	for comic := range comics {
		...
		for scanner.Scan() {
			...
			if _, ok := workIndex[token]; !ok {
				workIndex[token] = make(map[int]bool, 1)
			}
			workIndex[token][comic.Num] = true
		}
	}
	return workIndex, numIndex
}
...
// ./ex4.12 get 1
// ./ex4.12 index test.txt
// ./ex4.12 search test.txt "ha ha ha"
